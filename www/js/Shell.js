var dataStore = new Persist.Store("commandHistory"); //A persistent data store used to preserve console data across page changes/refreshes

function init() {
    //Initialize the shell
    initShell();
    //Load persistent data on loading the page.
    dataStore.get("commandHistoryLength", function(ok, val) {
        if (ok && val!=null)
            storedHistoryLength = val;
    });
    for(var i = 0; i<storedHistoryLength; i++) {
        dataStore.get("commandHistory"+i, function(ok, val) {
            tempValue = val;
        });
        if(tempValue == null)
            alert("Command history data store error: "+i)
        histList[i] = tempValue;
    }
    dataStore.get("commandHistoryPosition", function(ok, val) {
        if (ok && val!=null)
            histPos = val;
    });
}
...
// delete the command history.
  dataStore.set("commandHistoryLength", 0);
  dataStore.set("commandHistoryPosition", 0);
  histList.length = 1;
  histPos = 0;
  println("Command history data store deleted");
},
